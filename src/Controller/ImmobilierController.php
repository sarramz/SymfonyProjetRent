...
final class ImmobilierController extends AbstractController
{
    ...
    public function index(Request $request, ImmobilierRepository $immobilierRepository): Response
    {
        ...
        $nbrChambres = $request->query->get('nbr_chambres');

        // Construction de la requête avec les filtres
        $queryBuilder = $immobilierRepository->createQueryBuilder('i');

        if ($ville) {
            $queryBuilder->andWhere('i.ville LIKE :ville')
                ->setParameter('ville', '%' . $ville . '%');
        }
        if ($prixMin) {
            $queryBuilder->andWhere('i.prix >= :prixMin')
                ->setParameter('prixMin', $prixMin);
        }
        if ($prixMax) {
            $queryBuilder->andWhere('i.prix <= :prixMax')
                ->setParameter('prixMax', $prixMax);
        }
        if ($nbrChambres) {
            $queryBuilder->andWhere('i.nbr_chambres = :nbrChambres')
                ->setParameter('nbrChambres', $nbrChambres);
        }

        $immobiliers = $queryBuilder->getQuery()->getResult();
